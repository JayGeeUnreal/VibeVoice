...
class VibeVoiceChat:
    def __init__(self, model_path: str, device: str = "cuda", inference_steps: int = 5, default_voice_path: str = None):
        ...
        if torch.cuda.is_available():
            ...
            try:
                ...
                print("Warning: Could not set per-process memory fraction. Older PyTorch version or different CUDA setup?")
    ...
    def generate_audio_stream(
        ...
        try:
            ...
            self.is_generating = False
            self.current_streamer = None
            yield None # Yield None to signal failure to chat_fn
    ...
    with gr.Blocks(theme=gr.themes.Soft(primary_hue="blue", secondary_hue="purple"), fill_height=True) as interface:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=2):
                ...
                msg = gr.Textbox(
                    label="Message",
                    placeholder="Type your message or paste a script (e.g., 'Speaker 0: Hello! Speaker 1: Hi there!')",
                    lines=3
                )
        ...
        def update_ui_after_generation(message, history, voice_1, voice_2, num_speakers, cfg_scale):
            ...
            if not message.strip():
                # If message is empty, reset state and return original history, no audio, etc.
                return history, None, gr.update(visible=False), "", {"interactive": True}, {"visible": False, "interactive": False}

            history = history or []
            history.append({"role": "user", "content": message})

            # Define button states during generation
            ...
            if audio_path:
                ...
                # Return updated history, audio path, visible audio, clear message, and button states
                ...
                # Return updated history, no audio, hidden audio, clear message, and button states (even on failure)
        ...
        # Message submit event (Enter key)
...
def main():
    ...
    try:
        ...
        print("Please check your model path, dependencies, and hardware compatibility.")
        exit(1)
